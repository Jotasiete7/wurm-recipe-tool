...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-wurm-bg text-wurm-text selection:bg-wurm-accent selection:text-black">
      ...
      <div className="fixed top-4 right-4 z-50 flex gap-3 text-[10px] font-mono tracking-widest text-wurm-muted uppercase bg-black/50 p-1.5 rounded-full border border-wurm-border backdrop-blur-sm">
         <button onClick={() => setLang('en')} className={`px-2 py-0.5 rounded-full transition-all ${lang === 'en' ? 'bg-wurm-accent text-black font-bold' : 'hover:text-wurm-accent'}`}>EN</button>
         <button onClick={() => setLang('pt')} className={`px-2 py-0.5 rounded-full transition-all ${lang === 'pt' ? 'bg-wurm-accent text-black font-bold' : 'hover:text-wurm-accent'}`}>PT</button>
