...
veg jalfrezi,Hot Food Cooking,Panela,Forno Aberto,chopped garlic; chopped onion; cooking oil (25%); cornflour; passata (25%); rice; salt; vinegar (10%); ground cumin; ground paprika; ground ginger
...
function backupRecipes() {
    console.log("💾 Starting Legacy Backup...");

    const lines = RAW_CSV_DATA.trim().split('\n');
    console.log(`Parsing ${lines.length - 1} recipes...`);
    ...
    const jsonContent = JSON.stringify({
        meta: {
            ...
            note: "Backup of legacy RAW_CSV_DATA before database migration."
        },
        recipes: recipes
    }, null, 2);

    fs.writeFileSync('backup_legacy_recipes.json', jsonContent);
    console.log(`✅ Backup Saved: backup_legacy_recipes.json (${recipes.length} recipes)`);
}
